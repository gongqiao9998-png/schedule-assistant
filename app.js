...
async function checkTaggedMailboxMails() {
  ...
  try {
    ...
    preloadMailImportQueue(response);
    const queueMessage = buildLoadedQueueMessage();
    form.resultMessage = [buildTaggedMailResultMessage(response, form), queueMessage]
      .filter(Boolean)
      .join(" ");
    state.mailboxSettings = form;
    saveState();
    renderMailboxSettings();
    renderMailImport();
    if (state.mailImport?.mode === "parsed") {
      scrollToMailImport();
    }
    ui.voiceStatus.textContent = queueMessage || "最近 [助理] 邮件检查完成";
  } catch (error) {
...
function buildTaggedMailResultMessage(response, form) {
  ...
  const latest = response.items?.[0] || response.item;
  const latestSubject = latest?.item?.subject || latest?.subject || "";
  const queueCount = response.items?.length || 0;
  const latestSummary = latestSubject
    ? ` 最近一封是《${cleanImportedMailTitle(latestSubject, form.subjectTag)}》。`
    : "";
  const queueSummary = queueCount > 1 ? ` 已加载 ${queueCount} 封待处理邮件，可在导入区逐封处理。` : "";
  return `已找到 ${response.count} 封 ${form.subjectTag} 邮件。${latestSummary}${queueSummary}`.trim();
}

function preloadMailImportQueue(response) {
  const mailItems = (response?.items || [])
    .map((entry) => buildMailImportFromMailboxItem(entry.item, entry.parsed))
    .filter(Boolean);
  const fallbackItem = response?.item
    ? buildMailImportFromMailboxItem(response.item, response.parsed)
    : null;
  const queue = mailItems.length ? mailItems : fallbackItem ? [fallbackItem] : [];
...
function buildLoadedQueueMessage() {
  const importState = state.mailImport;
  const totalCount = importState?.items?.length || 0;
  if (!totalCount) {
    return "";
  }

  const currentIndex = Math.min((importState.currentIndex || 0) + 1, totalCount);
  return `已加载 ${totalCount} 封待处理邮件，当前第 ${currentIndex} 封。请前往“转发邮件导入”区逐封处理。`;
}
