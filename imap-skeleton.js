...
async function listTaggedMails(input, limit = 20) {
  ...
  try {
    ...
    const oldestSequences = [...allSequences].sort((left, right) => Number(left) - Number(right));
    const items = [];
    const maxMatches = Math.max(limit, 1);

    for (const sequence of oldestSequences) {
...
async function inspectTaggedMailsWithRaw(input, limit = 20) {
  const { config, errors } = validateMailboxConfig(input);
  if (errors.length) {
    throw makeError("INVALID_MAILBOX_CONFIG", errors.join(" "));
  }
